...
export async function answerRoutes(app: FastifyInstance) {
  ...
  app.get<{ Querystring: { asker_token: string; question_id: string } }>(
    '/asker/answers',
    {
    ...
    async (request, reply) => {
      ...
      return reply.send({
        ...
        ciphertext_answer: answer.ciphertextAnswer
          ...
          : null,
        nonce: answer.nonce ? toBase64Url(new Uint8Array(answer.nonce)) : null,
        dek_for_asker: answer.dekForAsker ? toBase64Url(new Uint8Array(answer.dekForAsker)) : null,
        created_at: answer.createdAt.toISOString(),
      });
    }
  );
  ...
  app.get<{ Params: { question_id: string } }>(
    '/owner/answers/:question_id',
    {
    ...
    async (request, reply) => {
      ...
      return reply.send({
        ...
        ciphertext_answer: answer.ciphertextAnswer
          ...
          : null,
        nonce: answer.nonce ? toBase64Url(new Uint8Array(answer.nonce)) : null,
        dek_for_owner: answer.dekForOwner ? toBase64Url(new Uint8Array(answer.dekForOwner)) : null,
        created_at: answer.createdAt.toISOString(),
        published_at: answer.publishedAt?.toISOString() || null,
      });
    }
  ...
  app.post<{ Params: { answer_id: string } }>(
